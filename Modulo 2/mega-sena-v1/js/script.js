...
function renderSavedGame(){
  var divSavedGame = document.querySelector('#mega-sena-saved-game');
  ...
  if (state.savedGame.length === 0) {
    ...
    for (var i = 0; i < state.savedGame.length; i++) {
      var currentGame = state.savedGame[1]
...
function addNumberToGame(numberToAdd){
  ...
  if (isNumberInGame(numberToAdd)){
    console.error('Este número já foi escolhido', numberToAdd);
    return;
  }
...
function removeNumberFromGame(numberToRemove){
  ...
  for(var i = 0; i < state.currentGame.length; i++){
    var currenteNumber = state.currentGame[i]
